    ...
    public void onMessageReceived(MessageReceivedEvent event){
        if (event.getMember().getId().equals(member.getId())){
            try {
                ...
                Conductor.getInstance().sendMessage("Please only enter whole numbers.", channel, delay);
            } catch (IndexOutOfBoundsException e) {
                Conductor.getInstance().sendMessage("Sorry, that number is not an option.", channel, delay);
            }
        }
    }
