...
public class ProfileUpdateConversation extends ListenerAdapter {
    ...
    public ProfileUpdateConversation(Member member, TextChannel channel, int ticketID){
        ...
        profile = Conductor.getInstance().getProfile(member);
    ...
    public void onButtonInteraction(ButtonInteractionEvent event){
        if (event.getMember().getId().equals(member.getId())){
            if (event.getButton().getId().equals(djNameBtn)){
                Conductor.getInstance().sendMessage(questions.get(0), channel, delay);
            }else if (event.getButton().getId().equals(vrcNameBtn)){
                Conductor.getInstance().sendMessage(questions.get(1), channel, delay);
                step = 1;
            }else if (event.getButton().getId().equals(genresBtn)){
                Conductor.getInstance().sendMessage(questions.get(2), channel, delay);
                step = 2;
                genresContext();
            }else if (event.getButton().getId().equals(socialsBtn)){
                Conductor.getInstance().sendMessage(questions.get(3), channel, delay);
                step = 3;
                socialsContext();
            }else if (event.getButton().getId().equals(logoBtn)){
                Conductor.getInstance().sendMessage(questions.get(4), channel, delay);
                step = 4;
            }else if (event.getButton().getId().equals(demosBtn)){
                Conductor.getInstance().sendMessage(questions.get(5), channel, delay);
                step = 5;
                demosContext();
                ...
                Conductor.getInstance().sendMessage("Let's try that again then.", channel, delay);
                Conductor.getInstance().sendMessage(questions.get(step), channel, delay);
    ...
    private void genresContext(){
        ...
        updater.start();
        Conductor.getInstance().getJDA().addEventListener(updater);
    }
